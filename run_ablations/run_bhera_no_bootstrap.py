...
from algs.bhera_no_bootstrap import BHERA
...
parser.add_argument("--no_bootstrap", action="store_true", help="Ablation: disable bootstrap critic (no ensemble uncertainty weighting / head bootstrap loss)")
...
def _init_worker(
    ...
    g_alg = BHERA(
        ...
        use_bootstrap=(not bool(no_bootstrap)),
        writer=None,
        global_step_ref=[0],
...
def main():
    ...
    alg = BHERA(
        ...
        use_bootstrap=(not bool(args.no_bootstrap)),
        writer=writer,
        global_step_ref=[0],
    ...
    pool = mp.Pool(
        ...
        initargs=(dire, env_config_path, schedule_path, worker_config_path,
                  ...
                  gamma, n_step, beta_init, kl_capacity, beta_lr, kl_ema_decay, bool(args.no_bootstrap))
...
if __name__ == "__main__":
    log_dir = f"../logs/ablations/no_bootstrap/" + time.strftime("%Y%m%d-%H%M%S")
