...
# ===== Model adapter: use BHERAJointModel as a drop-in for MAPPO =====
class BHERAAsMAPPOModel(torch.nn.Module):
    """Wrap BHERAJointModel to match the (typical) MAPPO/PPO model API used in this project.

    Key simplifications (as requested):
    - No belief inference: use z = 0
    - No cross-layer coupling: we instantiate BHERAJointModel with num_layers=1 and always use lid=0
    - Only keep Gaussian actor + scalar critic (use ensemble mean as value)
    """
    ...
    def __init__(
        ...
        super().__init__()
        self.task_input_dim = int(task_input_dim)
        self.worker_load_input_dim = int(worker_load_input_dim)
        self.worker_profile_input_dim = int(worker_profile_input_dim)
        self.n_worker = int(n_worker)
        self.num_pad_tasks = int(num_pad_tasks)

        # Flatten: task_queue + worker_loads + worker_profile -> raw_obs vector
        self.obs_dim = self.num_pad_tasks * self.task_input_dim + self.n_worker * self.worker_load_input_dim + self.n_worker * self.worker_profile_input_dim
        self.action_dim = self.n_worker  # one continuous action per worker

        # Map hidden_dim -> BHERA dims (keep small-ish, controlled by ppo_config['hidden_dim'])
        obs_embed_dim = int(hidden_dim)
        coupling_hidden = int(hidden_dim)
        policy_hidden = max(128, int(hidden_dim))
        value_hidden = max(128, int(hidden_dim))

        self.core = BHERAJointModel(
            obs_dim=self.obs_dim,
            action_dim=self.action_dim,
            num_layers=1,
            obs_embed_dim=obs_embed_dim,
            coupling_hidden=coupling_hidden,
            belief_in_dim=obs_embed_dim,      # unused (z=0), but keep consistent
            slow_window_len=16,
            slow_n_layers=1,
            slow_n_heads=4,
            slow_ff_dim=max(128, obs_embed_dim * 2),
            fast_hidden=obs_embed_dim,
            z_slow_dim=1,                     # keep dims >0 (but we will feed z=0, no belief used)
            z_fast_dim=1,
            policy_hidden=policy_hidden,
            value_hidden=value_hidden,
            value_ensemble=1,                 # single head is enough for MAPPO baseline
            decoder_hidden=policy_hidden,
        )

    def _pack_obs(self, task_obs, worker_loads, worker_profile):
        """Pack (task_queue, worker_loads, worker_profile) into a flat vector.

        Accepts either:
        - numpy arrays: [P, Dt], [W, Dl], [W, Dp]
        - torch tensors:
            [B, P, Dt] / [P, Dt], [B, W, Dl] / [W, Dl], [B, W, Dp] / [W, Dp]
        Returns torch.Tensor [B, obs_dim]
        """
        if not torch.is_tensor(task_obs):
            task_obs = torch.tensor(task_obs, dtype=torch.float32)
        if not torch.is_tensor(worker_loads):
            worker_loads = torch.tensor(worker_loads, dtype=torch.float32)
        if not torch.is_tensor(worker_profile):
            worker_profile = torch.tensor(worker_profile, dtype=torch.float32)

        # add batch dim if missing
        if task_obs.dim() == 2:
            task_obs = task_obs.unsqueeze(0)
        if worker_loads.dim() == 2:
            worker_loads = worker_loads.unsqueeze(0)
        if worker_profile.dim() == 2:
            worker_profile = worker_profile.unsqueeze(0)

        B = task_obs.shape[0]
        # ensure fixed sizes (pad/truncate defensively)
        P = task_obs.shape[1]
        W = worker_loads.shape[1]

        if P != self.num_pad_tasks:
            if P > self.num_pad_tasks:
                task_obs = task_obs[:, :self.num_pad_tasks, :]
            else:
                pad = torch.zeros((B, self.num_pad_tasks - P, task_obs.shape[2]), device=task_obs.device, dtype=task_obs.dtype)
                task_obs = torch.cat([task_obs, pad], dim=1)

        if W != self.n_worker:
            if W > self.n_worker:
                worker_loads = worker_loads[:, :self.n_worker, :]
                worker_profile = worker_profile[:, :self.n_worker, :]
            else:
                pad_l = torch.zeros((B, self.n_worker - W, worker_loads.shape[2]), device=worker_loads.device, dtype=worker_loads.dtype)
                pad_p = torch.zeros((B, self.n_worker - W, worker_profile.shape[2]), device=worker_profile.device, dtype=worker_profile.dtype)
                worker_loads = torch.cat([worker_loads, pad_l], dim=1)
                worker_profile = torch.cat([worker_profile, pad_p], dim=1)
    ...
    def _forward_features(self, task_obs, worker_loads, worker_profile):
        raw = self._pack_obs(task_obs, worker_loads, worker_profile)
        h = self.core.encode_obs(raw)  # [B, obs_embed_dim]
        z = torch.zeros((h.shape[0], self.core.z_dim), device=h.device, dtype=h.dtype)  # no belief used
        return h, z

    # --- API used by sampling ---
    def act(self, task_obs, worker_loads, worker_profile, valid_mask=None):
        h, z = self._forward_features(task_obs, worker_loads, worker_profile)
        action, logp, entropy = self.core.act(lid=0, h_tilde=h, z=z)
        _, v, _ = self.core.get_value_ensemble(lid=0, h_tilde=h, z=z)
        return v, action, logp, entropy

    def get_value(self, task_obs, worker_loads, worker_profile, valid_mask=None):
        h, z = self._forward_features(task_obs, worker_loads, worker_profile)
        _, v, _ = self.core.get_value_ensemble(lid=0, h_tilde=h, z=z)
        return v

    # --- API used by PPO update ---
    def evaluate_actions(self, task_obs, worker_loads, worker_profile, valid_mask=None, actions=None):
        if actions is None:
            actions = valid_mask
            valid_mask = None
        h, z = self._forward_features(task_obs, worker_loads, worker_profile)
        mean, std = self.core.forward_actor(lid=0, h_tilde=h, z=z)
...
class SimpleAgent:
    """Minimal agent wrapper to keep the sampling call-sites unchanged."""

    def __init__(self, alg, device):
        self.alg = alg
        self.device = device

    @torch.no_grad()
    def sample(self, task_obs, worker_loads, profile):
        self.alg.model.to(self.device)
        v, a, logp, ent = self.alg.model.act(
            torch.tensor(task_obs, dtype=torch.float32, device=self.device),
            torch.tensor(worker_loads, dtype=torch.float32, device=self.device),
            torch.tensor(profile, dtype=torch.float32, device=self.device),
        )
        return (
            v.squeeze(-1).cpu().numpy().item(),
            a.squeeze(0).cpu().numpy(),
            logp.squeeze(-1).cpu().numpy().item(),
            ent.squeeze(-1).cpu().numpy().item(),
        )
    ...
    for lid in range(g_num_layers):
        ...
        if alg_name.lower() == "mappo":
            ...
            alg = HAPPO(model, clip_param=0.2, value_loss_coef=0.5, entropy_coef=0.0,
                        ...
                        writer=None, global_step_ref=[0], total_training_steps=1, device="cpu")
        g_algs[lid] = alg
        g_agents[lid] = SimpleAgent(alg, device="cpu")
...
def main():
    ...
    for lid in range(num_layers):
        ...
        agents[lid] = SimpleAgent(alg, device)
        algs[lid] = alg
...
if __name__ == "__main__":
    log_dir = f'../logs/ablations/no_belief/{dire}/' + time.strftime("%Y%m%d-%H%M%S")
