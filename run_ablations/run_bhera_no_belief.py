...
from algs.bhera_no_belief import BHERA
...
parser.add_argument("--no_belief", action="store_true", help="Ablation: disable belief inference (z,q,KL,decoder) and feed zero latent")
...
def main():
    ...
    pool = mp.Pool(
        ...
        initargs=(dire, env_config_path, schedule_path, worker_config_path,
                  ...
                  gamma, n_step, beta_init, kl_capacity, beta_lr, kl_ema_decay, bool(args.no_belief))
...
if __name__ == "__main__":
    log_dir = f"../logs/ablations/no_belief/" + time.strftime("%Y%m%d-%H%M%S")
