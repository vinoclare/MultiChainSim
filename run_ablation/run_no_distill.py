...
def evaluate_policy(agent, eval_env, eval_episodes, writer, global_step, pid, device):
    ...
    for episode in range(eval_episodes):
        ...
        while not done:
            ...
            for lid in range(num_layers):
                ...
                action = agent.sample(obs_dict, pid)
...
for episode in range(num_episodes):
    ...
    if (episode % eval_interval == 0) and (episode > warmup_ep * K):
        evaluate_policy(agent, eval_env, eval_episodes, writer, global_step, distill_pid, device)
