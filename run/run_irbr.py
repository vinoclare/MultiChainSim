...
def load_offline_data_per_layer(root_dir: Path, num_layers: int, device):
    """
    root_dir: ../offline_data/xxx/{dire}/{dataset} 目录
    返回：
...
def evaluate_policy(actors, env, num_layers, state_dims, action_shapes,
    ...
    for _ in range(eval_episodes):
        ...
        while not done:
            ...
            for lid in range(num_layers):
                layer_obs = obs[lid]
                state_np = build_state_from_env_obs(layer_obs, state_dims[lid])
...
def train_irbr(args):
    ...
    # ---------- 每层 Actor / Critic / Target / Reference ----------
    actors = {}
    actor_targets = {}
    actor_refs = {}          # CPI 的 reference policy（冻结快照）
    ...
    for lid in range(num_layers):
        ...
        actor_ref = Actor(state_dim, action_dim, hidden_dim=args.hidden_dim, max_action=max_action).to(device)
        actor_ref.load_state_dict(actor.state_dict())  # 初始参考=当前策略
    ...
    # ---------- TensorBoard ----------
    log_dir = Path("../logs/irbr") / dire / time.strftime("%Y%m%d-%H%M%S")
    ...
    print(f"[train] start IRBR(CPI) offline training, total_updates={total_updates}")
    ...
    for it in range(0, total_updates + 1):
        ...
        # ----- Actor + target 更新（延迟，CPI/IRBR 核心改这里） -----
        if it % args.policy_delay == 0:
            for lid in range(num_layers):
                dataset = datasets[lid]
                actor = actors[lid]
                actor_ref = actor_refs[lid]
                critic = critics[lid]
                critic_target = critic_targets[lid]
                actor_target = actor_targets[lid]
                ...
                # 1) 先把“更新前”的 actor 快照存到 reference policy（冻结）
                #    这样 ref_loss 就是 D(pi_new, pi_old)，实现 conservative update
                actor_ref.load_state_dict(actor.state_dict())

                # 2) 常规 TD3+BC 的 Q 项（你原来那套 lambda = alpha / E|Q|）
                pi = actor(states)
                q_pi = critic.q1_only(states, pi)
                lambda_coef = args.alpha / (q_pi.abs().mean().detach() + 1e-6)

                # 3) CPI/IRBR 的两类正则：
                #    - 行为正则：靠近数据动作（近似 KL(pi || pi_D)）
                bc_loss = F.mse_loss(pi, actions)

                #    - 引用正则：靠近 reference policy（近似 KL(pi || pi_bar)）
                with torch.no_grad():
                    pi_ref = actor_ref(states)
                ref_loss = F.mse_loss(pi, pi_ref)

                # 4) 合成目标（论文式 (10) 的确定性近似版本）
                #    L = -lambda_coef * Q + tau * ( lam * ref + (1-lam) * bc )
                reg = args.lam * ref_loss + (1.0 - args.lam) * bc_loss
                actor_loss = (-lambda_coef * q_pi.mean() + args.tau * reg)
                ...
                # soft update target
                soft_update(critic_target, critic, args.tau_target)
                soft_update(actor_target, actor, args.tau_target)

                actor_losses_step.append(actor_loss.item())
                lambdas_step.append(lambda_coef.item())
                bc_losses_step.append(bc_loss.item())
                ref_losses_step.append(ref_loss.item())
        ...
        if it % args.log_interval == 0:
            ...
            mean_bc = float(np.mean(bc_losses_step)) if bc_losses_step else 0.0
            mean_ref = float(np.mean(ref_losses_step)) if ref_losses_step else 0.0
            ...
            print(
                ...
                f"bc_mse_mean={mean_bc:.4f} "
                f"ref_mse_mean={mean_ref:.4f} "
            ...
            if actor_losses_step:
                ...
                writer.add_scalar("train/bc_mse_mean", mean_bc, it)
                writer.add_scalar("train/ref_mse_mean", mean_ref, it)
    ...
    ckpt_path = ckpt_dir / "irbr_best.pt"
...
def main():
    ...
    parser.add_argument(
        ...
        help="root dir of offline data, final dir = root/dire/dataset",
    )

    parser.add_argument(
        "--ckpt_root",
        type=str,
        default="./checkpoints/irbr",
        help="where to save irbr checkpoints",
    )
    ...
    # TD3+BC 的 alpha（用于 lambda_coef = alpha / E|Q|）
    parser.add_argument("--alpha", type=float, default=0.25)

    # ===== IRBR / CPI 超参 =====
    # tau：正则强度；lam：ref vs data 的权重
    parser.add_argument("--tau", type=float, default=1.0, help="IRBR regularization strength")
    parser.add_argument("--lam", type=float, default=0.7, help="mixing weight: ref (lam) vs data (1-lam)")
