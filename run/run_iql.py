...
def _load_layer_transitions(npz_files, lid: int):
    """
    Load and concat transitions for one layer from your expert npz format.

    Keys (per run_crescent_expert_data.py):
      l{lid}_task_obs, l{lid}_worker_loads, l{lid}_worker_profile, l{lid}_valid_mask
      l{lid}_actions
      l{lid}_rewards, l{lid}_dones
...
def _sample_batch(layer_data: dict, batch_size: int, action_flat_dim: int, device: str):
...
class StateEncoder(nn.Module):
    """
    Flatten (task_obs, valid_mask, worker_loads, worker_profile) -> z
    """
    def __init__(self, in_dim, hidden_dim=256, z_dim=256):
        super().__init__()
...
class VNet(nn.Module):
    def __init__(self, encoder: StateEncoder, hidden_dim=256):
        super().__init__()
        self.encoder = encoder
        z_dim = encoder.net[-1].out_features
        self.v = mlp(z_dim, hidden_dim, 1, depth=2)

    def forward(self, task_obs, worker_loads, worker_profile, valid_mask):
        z = self.encoder(task_obs, worker_loads, worker_profile, valid_mask)
        return self.v(z).squeeze(-1)  # [B]


class TanhGaussianPolicy(nn.Module):
    """
    Tanh-squashed Gaussian, then rescale to action bounds.
    IQL actor uses log_prob(a|s) with tanh correction.
    """
    def __init__(self, encoder: StateEncoder, action_dim: int, hidden_dim=256,
...
# =========================
class IQL:
    def __init__(self, q1, q2, v, pi,
                 action_low, action_high,
                 gamma=0.99, expectile_tau=0.7, beta=3.0, max_weight=100.0,
                 lr=3e-4, target_tau=0.005):
        self.q1, self.q2 = q1, q2
        self.v, self.pi = v, pi

        self.q1_t = QNet(copy_encoder(q1.encoder), action_dim=action_low.numel(), hidden_dim=256).to(action_low.device)
        self.q2_t = QNet(copy_encoder(q2.encoder), action_dim=action_low.numel(), hidden_dim=256).to(action_low.device)
    ...
    def update(self, batch):
        ...
        # ---- Q loss ----
        with torch.no_grad():
            v_next = self.v(ns_task, ns_load, ns_prof, ns_mask)
            y = r + self.gamma * (1.0 - d) * v_next

        q1 = self.q1(s_task, s_load, s_prof, s_mask, a)
        q2 = self.q2(s_task, s_load, s_prof, s_mask, a)
        ...
        with torch.no_grad():
            tq1 = self.q1_t(s_task, s_load, s_prof, s_mask, a)
            tq2 = self.q2_t(s_task, s_load, s_prof, s_mask, a)
            qmin = torch.min(tq1, tq2)

        v = self.v(s_task, s_load, s_prof, s_mask)
        diff = qmin - v
        w = torch.where(diff > 0, self.tau, 1.0 - self.tau)
        ...
        # ---- Policy loss (adv-weighted loglik) ----
        with torch.no_grad():
            v_det = self.v(s_task, s_load, s_prof, s_mask)
            adv = qmin - v_det
            weights = torch.exp(adv / max(self.beta, 1e-6)).clamp(max=self.max_weight)
        ...
        info = {
            ...
            "v_mean": float(v.detach().cpu().mean().item()),
    ...
    def act(self, task_obs, worker_loads, worker_profile, valid_mask, sample=True):
        ...
        # 你环境里 mean-only 经常不行：这里 deterministic 也直接 sample 一次（更符合你经验）
...
def copy_encoder(enc: StateEncoder) -> StateEncoder:
    # create a same-shaped encoder without sharing params
    # (we will load_state_dict later via QNet copy anyway)
    n = enc
    # We cannot easily recover dims from modules; keep it simple:
    # We'll deep-copy by state_dict usage through QNet; so here just return a deepcopy-like new net is hard.
    # Instead: rebuild via python attribute inference is messy. We'll do a real deepcopy of whole encoder.
    import copy
    return copy.deepcopy(enc)


# =========================
# Agent wrapper (match IndustrialAgent.sample signature)
# =========================
class IQLAgent:
    def __init__(self, alg: IQL, action_shape, device="cuda"):
        self.alg = alg
        self.action_shape = tuple(action_shape)
        self.device = torch.device(device)

    def sample(self, task_obs, worker_loads, worker_profile):
        # valid_mask is always task_obs[:,3] in your codebase
        ...
        a = self.alg.act(t, l, p, m, sample=True)  # [1, action_dim]
        a_np = a.detach().cpu().numpy().reshape(self.action_shape)

        # Keep same return format: (value, action, logprob, info)
        return 0.0, a_np, 0.0, None


# =========================
# Eval (copy from run_crescent.py)
# =========================
...
def main():
    ...
    parser.add_argument("--log_root", type=str, default="../logs/iql")
    ...
    # Build env just to get spaces + worker_config, then build eval_env with same worker_config (same as run_crescent.py)
    ...
    # Assume continuous Box action
    if not hasattr(act_space, "low") or not hasattr(act_space, "high"):
        raise TypeError(f"IQL baseline expects continuous Box action space, got: {type(act_space)}")

    n_slots = int(obs_space["task_queue"].shape[0])
    task_dim = int(obs_space["task_queue"].shape[1])
    n_worker = int(obs_space["worker_loads"].shape[0])
    load_dim = int(obs_space["worker_loads"].shape[1])
    ...
    print(f"[IQL] dire={dire} data_dir={data_dir} npz_files={len(files)} layers={num_layers}")
    print(f"[IQL] obs: n_slots={n_slots} task_dim={task_dim} n_worker={n_worker} load_dim={load_dim} profile_dim={profile_dim}")
    print(f"[IQL] act: shape={action_shape} flat_dim={action_flat_dim}")

    layer_data = {}
    for lid in range(num_layers):
        layer_data[lid] = _load_layer_transitions(files, lid)
        print(f"[IQL] layer {lid} transitions = {layer_data[lid]['rewards'].shape[0]}")
    ...
    # Build per-layer IQL (baseline: independent)
    algs = {}
    agents = {}
    for lid in range(num_layers):
        enc_q = StateEncoder(enc_in_dim, hidden_dim=args.hidden_dim, z_dim=args.z_dim).to(device)
        enc_v = StateEncoder(enc_in_dim, hidden_dim=args.hidden_dim, z_dim=args.z_dim).to(device)
        enc_pi = StateEncoder(enc_in_dim, hidden_dim=args.hidden_dim, z_dim=args.z_dim).to(device)

        q1 = QNet(enc_q, action_dim=action_flat_dim, hidden_dim=args.hidden_dim).to(device)
        q2 = QNet(copy_encoder(enc_q), action_dim=action_flat_dim, hidden_dim=args.hidden_dim).to(device)
        v = VNet(enc_v, hidden_dim=args.hidden_dim).to(device)
        pi = TanhGaussianPolicy(enc_pi, action_dim=action_flat_dim, hidden_dim=args.hidden_dim).to(device)

        alg = IQL(
            q1=q1, q2=q2, v=v, pi=pi,
        ...
        algs[lid] = alg
        agents[lid] = IQLAgent(alg, action_shape=action_shape, device=str(device))

    # Logging
    log_dir = Path(args.log_root) / dire / time.strftime("%Y%m%d-%H%M%S")
    writer = SummaryWriter(str(log_dir))
    print(f"[IQL] log_dir = {log_dir}")

    t0 = time.time()
    for step in range(1, args.updates + 1):
        for lid in range(num_layers):
            batch = _sample_batch(layer_data[lid], args.batch_size, action_flat_dim, str(device))
            info = algs[lid].update(batch)

            # if step % 200 == 0:
            #     writer.add_scalar(f"train/l{lid}_q_loss", info["q_loss"], step)
            #     writer.add_scalar(f"train/l{lid}_v_loss", info["v_loss"], step)
            #     writer.add_scalar(f"train/l{lid}_pi_loss", info["pi_loss"], step)
            #     writer.add_scalar(f"train/l{lid}_q_mean", info["q_mean"], step)
            #     writer.add_scalar(f"train/l{lid}_v_mean", info["v_mean"], step)
        ...
        if step % args.eval_interval == 0:
            ...
            print(f"[IQL] step={step}/{args.updates} elapsed={elapsed:.1f}s eval...")
            evaluate_policy(agents, eval_env, args.eval_episodes, writer, step)

    writer.close()
    print(f"[IQL] done. logs at: {log_dir}")
