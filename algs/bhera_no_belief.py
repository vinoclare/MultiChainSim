...
class BHERA:
    ...
    def _rebuild_joint_latent(self, obs_raw, actions, rewards, dones):
        ...
        # ===== Ablation: without belief =====
        # Disable slow/fast belief inference and provide a zero latent to match the actor/critic interface.
        if not self.use_belief:
            z_dim = int(self.model.z_slow_dim + self.model.z_fast_dim)
            z_seq = torch.zeros((B, T, z_dim), device=device)
            q_seq = torch.full((B, T, 1), 0.5, device=device)
    ...
    def sample_joint(
        ...
        if not self.use_belief:
            ...
            z_dim = int(self.model.z_slow_dim + self.model.z_fast_dim)
            z_t = torch.zeros((B, z_dim), device=self.device)
            act_list, logp_list, ent_list, v_list = [], [], [], []
            for lid in range(L):
                s_flat = obs_raw_bl[:, lid]
                a, logp, ent = self._call_forward_actor(lid, s_flat, z_t)
                v_ens = self._call_value_ensemble(lid, s_flat, z_t)
                v_mean = v_ens.mean(dim=-1, keepdim=True)
                act_list.append(a)
                logp_list.append(logp)
                ent_list.append(ent)
                v_list.append(v_mean)
            actions_out = torch.stack(act_list, dim=1)
            logp_out = torch.stack(logp_list, dim=1)
            ent_out = torch.stack(ent_list, dim=1)
            v_out = torch.stack(v_list, dim=1)
            q_out = torch.full((B, 1), 0.5, device=self.device)
            return actions_out, logp_out, ent_out, v_out, q_out, h_fast_prev
    ...
    def learn_joint(
        ...
        for _ in range(max(1, self.inner_k)):
            ...
            q_sup_loss = None
            if not self.use_belief:
                q_sup_loss = torch.tensor(0.0, device=self.device)
            if self.use_belief and q_labels is not None and getattr(self, "q_supervised_coef", 0.0) > 0:
