...
class BHERA:
    ...
    def __init__(
        ...
        self.use_regime_balance = bool(use_regime_balance)
    ...
    def learn_joint(
        ...
        for _ in range(max(1, self.inner_k)):
            ...
            if self.use_regime_balance:
                w_q = (1.0 + self.lambda_q * q_flat).clamp(min=0.0)  # [B*T]
            else:
                w_q = torch.ones_like(q_flat)
            ...
            if self.use_regime_balance and self.decoder_sparse_weight > 0:
