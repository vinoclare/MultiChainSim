...
class MuSE2(nn.Module):
    ...
    def collect(self, obs_dict, actions, pid):
        """
        收集一批数据，obs_dict 包含 task_obs、worker_loads、worker_profiles、valid_mask
    ...
    def bc_update(self, cur_pid: int, batch_size: int = 512, steps: int = 50):
        ...
        for _ in range(steps):
            ...
            # ---- 构造张量 ----
            obs_keys = ["task_obs", "worker_loads", "worker_profiles", "valid_mask"]
            ...
            mean_s, std_s, _, _ = self.model(
                obs_t["task_obs"], obs_t["worker_loads"], obs_t["worker_profiles"], obs_t["valid_mask"],
                policy_id=self.K
            )
